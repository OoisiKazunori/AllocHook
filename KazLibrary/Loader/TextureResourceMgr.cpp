...
KazBufferHelper::BufferData TextureResourceMgr::LoadGraphBuffer(std::string RESOURCE)
{
	...
	cpuBufferArray[elementNum] = KazBufferHelper::SetShaderResourceBufferData(textureDesc, RESOURCE);
	...
	if (isDDSFile)
	{
		LoadDDSFile(textureDesc, elementNum, RESOURCE);
	}
	else
	{
		cpuBufferArray[elementNum].bufferWrapper->GetBuffer()->WriteToSubresource
...
void TextureResourceMgr::LoadDDSFile(CD3DX12_RESOURCE_DESC arg_textureDesc, int arg_elementNum, std::string arg_filepass)
{
	//MipMapを取得。
